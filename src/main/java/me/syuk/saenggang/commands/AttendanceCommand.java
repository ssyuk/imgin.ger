...
public class AttendanceCommand implements Command {
    ...
    public void execute(DBManager.Account account, String[] args, Message message) {
        ...
        else if (status.ranking() == 2) account.giveCoin(message.getChannel(), 7, "출석 2등 보상으로");
        else if (status.ranking() == 3) account.giveCoin(message.getChannel(), 5, "출석 3등 보상으로");
        else account.giveCoin(message.getChannel(), 2, "출석 보상으로");
    }
}
