        ...
        if (WordRelay.isPlaying(user)) {
            ...
            CompletableFuture.runAsync(() -> {
                ...
                if (finalContent.length() == 1) {
                    ...
                    thinking.edit("이미 나온 단어에요! 제가 이겼네요!");
                    game.end(event.getChannel(), false);
                    return;
                ...
                JsonObject nextWord = game.getNextWord(finalContent);
                if (nextWord == null) {
                    thinking.edit("더이상 생각나는게 없어요.. <@" + user.getIdAsString() + ">님이 이겼네요!");
                    game.end(event.getChannel(), true);
                    return;
                }
                game.inputWord(nextWord.get("word").getAsString());
                String meaning = nextWord.getAsJsonArray("sense").get(0).getAsJsonObject().get("definition").getAsString();
